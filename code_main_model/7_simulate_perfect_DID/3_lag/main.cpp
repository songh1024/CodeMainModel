...
int main()
{
	...
	{
		if (bank_opt[ii_mkt] >= nyears || bank_opt[ii_mkt] < lag)
			continue;
		...
		for (int i_t = 0; i_t < period_trans; ++i_t)
			for (int i_mkt = 0; i_mkt < nmkt; ++i_mkt)
			{
				...
				{
					if ( ( bank_opt[j_mkt] <= i_t && j_mkt != ii_mkt && (access[j_mkt] == 1 || i_mkt == j_mkt) )
						|| ( bank_opt[j_mkt] <= i_t + lag && j_mkt == ii_mkt && (access[j_mkt] == 1 || i_mkt == j_mkt) ) )

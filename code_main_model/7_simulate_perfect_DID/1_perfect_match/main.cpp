...
int main()
{
	...
	int** comm_loc_model = ini_int_matrix2(nyears, nmkt);
	filename = inputpath + ("comm_loc_model.dat");
	read_int_dat2(comm_loc_model, nyears, nmkt, filename);
	...
	write_int2(filename, comm_loc_model, nyears, nmkt);
	...
	for (int ii_mkt = 0; ii_mkt < nmkt; ++ii_mkt)
	{
		...
		for (int i_t = 0; i_t < nyears; ++i_t)
			for (int i_mkt = 0; i_mkt < nmkt; ++i_mkt)
			{
				d[i_t][i_mkt] = 1e10;
				for (int j_mkt = 0; j_mkt < nmkt; ++j_mkt)
				{
					if (comm_loc_model[i_t][j_mkt] == 1 && j_mkt != ii_mkt && (access[j_mkt]==1 || i_mkt==j_mkt) )
						d[i_t][i_mkt] = min_2num(d[i_t][i_mkt], distance[i_mkt][j_mkt]);
				}
			}
		for (int i_t = nyears; i_t < period_trans; ++i_t)
			for (int i_mkt = 0; i_mkt < nmkt; ++i_mkt)
				d[i_t][i_mkt] = d[nyears-1][i_mkt];

	...
	double* gridb, double* gridtheta, double* gridz, double* probz, double** trans_z, double r)
{
	...
	for (int i_b=0; i_b<nb; ++i_b)
		for (int i_z=0; i_z<nz; ++i_z)
			...
			{
				//if (it == 1 && i_z == 0  && i_b == 0 && i_theta == 0)
				//	cout << endl;
				golden_section_search_ad(seg_nmkt, seg_mktidx,
					it, i_mkt, i_z, i_b, i_theta, theta_candidate, V_ad, gc_ad, gtheta_ad, gb_ad, V_prv, wealth_ad, gridb, gridtheta, gridz, probz, trans_z);
				golden_section_search_noad(seg_nmkt, seg_mktidx,
					it, i_mkt, i_z, i_b, i_theta, V_noad, gc_noad, gtheta_noad, gb_noad, V_prv, wealth_noad, gridb, gridtheta, gridz, probz, trans_z, r);
				//if (V_ad[i_z][i_b][i_theta] < -1e5 && V_noad[i_z][i_b][i_theta] < -1e5)
				//	cout << endl;
	...
	for (int index=0; index<coefficient; ++index) 
	{
		...
		while (itc<maxit_gss && diffc>=tol_gss)
		{
			...
			if (flag_c!=2)
			{
				...
				v_clow=value_ad(seg_nmkt, seg_mktidx, 
					i_mkt, state_b, state_z, total_wealth, consumption, theta, V_prv, gridb, gridtheta, trans_z);
			}

			if (flag_c!=1)
			{
				consumption=temp_chigh;
				v_chigh=value_ad(seg_nmkt, seg_mktidx, 
					i_mkt, state_b, state_z, total_wealth, consumption, theta, V_prv, gridb, gridtheta, trans_z);
			}
	...
	double* gridb, double* gridtheta, double* gridz, double* probz, double** trans_z, double r)
{
	...
	while (itc<maxit_gss && diffc>=tol_gss)
	{
		...
		{
			consumption=temp_clow;
			v_clow=value_noad(seg_nmkt, seg_mktidx, 
				i_mkt, state_b, state_z, total_wealth, deposit, consumption, V_prv, gridb, gridtheta, trans_z);
		}

		if (flag_c!=1)
		{
			consumption=temp_chigh;
			v_chigh=value_noad(seg_nmkt, seg_mktidx, 
				i_mkt, state_b, state_z, total_wealth, deposit, consumption, V_prv, gridb, gridtheta, trans_z);
		}
	...
	int i_mkt, int state_b, int state_z, double wealth, double consumption, double theta, double**** V_prv, double* gridb, double* gridtheta, double** trans_z)
{
	...
	else
	{
		for (int i_z_next = 0; i_z_next < nz; ++i_z_next)
		{
			...
			{
				if (seg_mktidx[j_mkt] == i_mkt)
					temp += exp(1 / eta * lin_interpo_2(nb, ntheta, gridb, gridtheta, V_prv[seg_mktidx[j_mkt]][i_z_next], b_next_NM, theta_next_NM));
				else
					temp += exp(1 / eta * lin_interpo_2(nb, ntheta, gridb, gridtheta, V_prv[seg_mktidx[j_mkt]][i_z_next], b_next_M, theta_next_M));
			}
			//if (temp == 0 || isinf(temp))
			//	cout << endl;
